...
public class SuggestionView extends FrameLayout {
    ...
    public SuggestionView(@NonNull final Context context, Sugerencia sugerencia) {
        super(context);
        ...
        textView.setText(sugerencia.getTipoNegocio());

        ImageView image = (ImageView)findViewById(R.id.img_category);
        String imageName = getImageName(sugerencia);
        int id = getResources().getIdentifier("com.quickmatch:drawable/" + imageName , null, null);
        image.setImageResource(id);

    }

    public String getImageName (Sugerencia sugerencia){
        switch (sugerencia.getIdCategoria()){
            case "1" :
                return "img_birria";
            case "2" :
                return "img_buffette";
